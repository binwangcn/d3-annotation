(function (global, factory) {
  ...
  const AnnotationCollection = function () {
  ...
  var addHandles = function addHandles(_ref5) {
    ...
    .attr("class", "handle").attr("fill", "grey").attr("fill-opacity", 0.1)
    .attr("cursor", "move").attr("stroke-dasharray", 5).attr("stroke", "grey")
    .call(d3Drag.drag().container(d3Selection.select("g.annotations").node())
    .on("start", function (evt,d) {
      return d.start && d.start(evt,d);
    }).on("drag", function (evt,d) {
      return d.drag && d.drag(evt,d);
    }).on("end", function (evt,d) {
      return d.end && d.end(evt,d);
    }));
  ...
  var connectorCurve = (function (_ref) {
    ...
    if (type.editMode) {
      ...
      var updatePoint = function updatePoint(index,event) {
  ...
  var subjectCircle = (function (_ref) {
    ...
    if (type.editMode) {
      ...
      var updateRadius = function updateRadius(attr,event) {
    ...
    createClass(Type, [{
      ...
      value: function drawNote() {
        ...
        if (this.editMode) {
          ...
          this.note.call(d3Drag.drag().container(d3Selection.select("g.annotations").node())
          .on("start", function (evt,d) {
            return start(evt,d);
          }).on("drag", function (evt,d) {
            return dragging(evt,d);
          }).on("end", function (evt,d) {
            return end(evt,d);
          }));
        } else {
